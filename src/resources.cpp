...
bool Resources::LoadFont(const std::string &fontName, ZipFileReader *zipFile)
{
    ...
    // Load the data from ResourceIO if present
    bool loadedUsingResourceIO = false;
    if (m_resourceIO) {
        const std::optional<std::string> queryResult = m_resourceIO->QueryFont(fontName);
        bool loadFailed = true;
        if (queryResult) {
            pugi::xml_parse_result parseResult = doc.load_buffer(queryResult->c_str(), queryResult->size());
            if (parseResult) {
                loadedUsingResourceIO = true;
            }
        }
    }

    if (!loadedUsingResourceIO) {
    ...
    for (pugi::xml_node current = root.child("g"); current; current = current.next_sibling("g")) {
        ...
        bool loadedUsingResourceIO = false;
        if (m_resourceIO) {
            std::optional<std::string> maybeXml = m_resourceIO->QueryGlyph(fontName, glyph.GetCodeStr());
            if (maybeXml) {
                glyph.SetXML(*maybeXml);
                loadedUsingResourceIO = true;
            }
        }
